...
fun intersect (v1,v2) =
    case (v1,v2) of
     ...
     | (_, NoPoints) => NoPoints (* 4 additional cases, because (NoPoints, NoPoints) already covered *) 
...
fun preprocess_prog e=
	...
	if real_close_point (x1,y1) (x2, y2) then Point(x1,y1) (* worked *)
	else
	    if real_close(x1,x2) then
		 (if y2 < y1 then LineSegment(x2,y2,x1,y1) else LineSegment(x1,y1,x2,y2))
	     else if x1 > x2
